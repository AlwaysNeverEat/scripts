...
> **Перенос масляного фильтра по двигателю.** После подбора скрипт разносит
> найденные масляные фильтры на машины с тем же двигателем — включая другие
> модели и марки концерна. Подпись двигателя: код (от четырёх символов) плюс
> объём плюс мощность.
>
> Переносится ТОЛЬКО масляный. Он часть мотора: посадочное место и резьба
> заданы блоком, и вместе с мотором он переезжает в любой кузов. Воздушный и
> салонный определяются кузовом, у разных моделей они разные даже при одном
> моторе — скопировать их значит выдать неверный артикул с той же
> уверенностью, что и верный.
>
> Расхождения не угадываются: если у одной подписи Mann вернул разные масляные
> фильтры, подпись пропускается целиком. Отключить перенос — `--no-engine-copy`.
