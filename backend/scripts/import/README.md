...
## Склейка дублей марок и моделей

```bash
node scripts/import/merge-duplicates.js --dry-run   # посмотреть план
node scripts/import/merge-duplicates.js             # применить
```

Единая точка правды о канонических написаниях — `shared/carCanon.js`:
«GWM HAVAL»/«HAVAL» → «Haval», «VW» → «Volkswagen», «LADA / AVTOVAZ» → «Lada»,
Ford «Focus Mk II» → «Focus» + поколение «2 (C307)». Марки, различающиеся
только регистром, приводятся к самому частому написанию. Если после
переименования две записи совпали по уникальному ключу — это один автомобиль:
выживает ручная запись (при нескольких — старейшая), её заполненные поля
неприкосновенны, из дублей доливается только пустое (объёмы, фильтры,
мощность) и объединяются допуски/теги/ссылки; car_events дублей переносятся
на выжившую, расхождения перечисляются в событии склейки и в логе.

Скрипт идемпотентен. Импортёры (`import-cars.js`, `apply-*.js`,
`build-enrichment-workset.js`) и сайт (POST/PATCH /api/cars) прогоняют
идентичность через те же правила, так что склеенные дубли не появятся заново.
Новый двойник — добавь alias в `BRAND_CANON` и прогони скрипт ещё раз.
