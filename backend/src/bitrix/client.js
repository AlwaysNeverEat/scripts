...
// ── Вход ─────────────────────────────────────────────────────────────────────
// Портал перекидывает на Bitrix24.Network, где пароль проверяет ajax-ручка
// b24network.authorize.check. После неё возвращаемся на портал по redirect_uri,
// и уже портал выдаёт свои куки.
//
// Ответ, который мы не поняли, считается ОТКАЗОМ, а не успехом: соврать
// «вошли» хуже, чем переспросить пароль.

const NET_AJAX = '/bitrix/services/main/ajax.php';

// Тело запроса снято с бандла самой формы (logging-in-scenario):
//
...
async function performLogin(login, password) {
    ...
    // 2. Пароль.
    //
    // Ступень «проверить логин» (verifyLogin / checkLogin) намеренно
    // пропущена. Форме она нужна, чтобы решить, что рисовать дальше — поле
    // пароля или код из СМС, — а нам рисовать нечего: мы либо входим паролем,
    // либо не входим вовсе. Каждая лишняя ступень здесь это ещё одно тело
    // запроса, которое надо угадать, и ещё одна причина получить «Wrong
    // request» на ровном месте. Он может быть каким угодно, включая кириллицу: тело уезжает в
