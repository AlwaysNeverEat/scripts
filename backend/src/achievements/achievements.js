...
//
// metric:
//   added      — число машин, добавленных пользователем (car_events
//                type='added'; у машины ровно одно такое событие,
//                переназначение переписывает его user_id, поэтому счётчик
//                честный).
//   edited     — число ОТРЕДАКТИРОВАННЫХ МАШИН, не правок: DISTINCT car_id.
//                100 правок одной машины = 1 машина.
//   registered — константа 1: сам факт существования аккаунта. Ачивки на этой
//                метрике есть у всех и никогда не отзываются. Существующим
//                пользователям её докидывает самолечащийся синк в
//                GET /profile/achievements/pending (первый же визит на сайт).
// ─────────────────────────────────────────────────────────────────────────────
...
export async function computeMetrics(userId) {
  ...
  // registered — константа: раз метрики считаются для существующего юзера,
  // значит он зарегистрирован (ачивка «Зеленый свет.»).
  return { added: r.rows[0].added, edited: r.rows[0].edited, registered: 1 };
}
