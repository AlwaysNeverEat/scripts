...
//       Устаревший отпечаток панель не роняет: сертификат, проходящий обычную
//       проверку, пройдёт и мимо него (см. pinnedConnector).
...
//
// Отдельно — САМЫЙ ЧАСТЫЙ случай несовпадения, и он не про подмену сервера.
// Отпечаток живёт до перевыпуска сертификата, а перевыпуск на стороне CRM
// случается сам собой (у certbot — раз в три месяца). Пин при этом ОТВЕРГАЛ
// сертификат, который прошёл бы обычную строгую проверку: на CRM всё починили,
// а панель у нас умерла, и вылечить её мог только человек, помнящий про строку
// в deploy/.env. Так быть не должно. CRM_TLS_FINGERPRINT — это ПОСЛАБЛЕНИЕ
// (пускаем просроченный и самоподписанный, если это он), и хуже, чем без него,
// с ним быть не может: сертификат, проходящий проверку сам по себе, пропускаем
// и с устаревшим пином, а в лог пишем, что строку пора убрать.
export function pinnedConnector(connect, fingerprints) {
    return (options, cb) => connect(options, (err, socket) => {
        ...
        // authorized проставлен, даже когда rejectUnauthorized снят, и значит
        // он ровно то же, что проверил бы Node без всяких CRM_TLS_*: цепочка
        // плюс совпадение имени хоста. Кэш TLS-сессий выключен (см.
        // buildTransport), так что посчитан он здесь по-настоящему, а не
        // унаследован от возобновлённой сессии.
        if (socket?.authorized === true) {
            warnOnce(`сертификат CRM перевыпущен (${fp.slice(0, 16)}…) и теперь проходит обычную`
                + ' проверку. CRM_TLS_FINGERPRINT в deploy/.env устарел — его пора убрать;'
                + ' пока он там, соединение держится на обычной проверке, а не на отпечатке');
