...
// кроме самого игрока, не нужна. В базу попадает только результат — победа
// (см. store.js), из неё и считается месячный топ.
//
// Что это значит на практике: перезапуск бэкенда обрывает начатые партии.
// Это нормально — сапёр начинается заново одним кликом, а вот выигранные
// партии переживут что угодно.
