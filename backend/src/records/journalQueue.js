...
// невыполненное, и показывает исход в самой вкладке: запись встала — доска
// перечитывается; CRM отказала — красная плашка в шапке раздела, а в списке
// «Очередь» строка с причиной, клиентом, станцией, временем и ответственным.
//
    ...
    return {
        enqueue(userId, { type, payload, author = '', group = null, note = null }) {
            ...
            const op = {
                ...
                // Подпись для списка «Очередь» (routes/journal.js → cleanNote).
                note,
        ...
        // Свежие первыми — так их рисует список «Очередь». Список ОБЩИЙ, как
        // был у старой очереди: смысл его в том, чтобы через час найти, кто,
        // кого, куда и на какое время записывал, — а записывают в одну доску
        // все. Чужие ставящиеся записи заодно рисуются на доске призраками, и
        // на занятый ими пост не записывает второй оператор. `mine` — своё:
        // только своё можно отменить, и только свой отказ висит плашкой.
        list(userId) {
            prune();
            return ops.slice(-LIST_MAX).reverse().map(o => ({ ...view(o), mine: o.userId === userId }));
        },
