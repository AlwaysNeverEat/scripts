...
// наружу из состояния партии уезжает id снимка и НИЧЕГО про место, а догадка
...
function view(state, seat) {
    ...
    return {
        ...
        max_hp: DUEL_HP,
        // Снимок отдаём только пока раунд идёт: после него он уже в итоге.
 ...
 * снимков — сеть, он может не ответить, и лучше отказать на кнопке «принять»,
