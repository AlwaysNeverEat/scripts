// Таблица очков морского боя: одна строка на игрока — победы и поражения
// (см. db/migrations/035_battleship.sql).
//
    ...
    const [top, mine] = await Promise.all([
        ...
        query(
            `SELECT s.wins, s.losses,
                    (SELECT count(*) FROM battleship_scores o
                      WHERE o.wins > s.wins
...
export async function addResult(winnerId, loserId) {
    ...
    await query(
        `INSERT INTO battleship_scores (user_id, wins, losses, played_at)
              VALUES ($1, 1, 0, now()), ($2, 0, 1, now())
         ON CONFLICT (user_id) DO UPDATE SET
              wins      = battleship_scores.wins   + EXCLUDED.wins,
              losses    = battleship_scores.losses + EXCLUDED.losses,
              played_at = now()`,
        [winnerId, loserId],
    );
}
