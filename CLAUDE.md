...
проигрыша. Раскладка мин живёт в памяти процесса (перезапуск бэкенда обрывает
начатые партии — это нормально) и клиенту не отдаётся до конца партии, иначе
мини-топ рисовался бы в консоли. В базу попадает только победа —
`minesweeper_wins` (миграция `025`), из неё считается месячный мини-топ внутри
окна игры. Мерка месяца та же, что у топа по записям.
