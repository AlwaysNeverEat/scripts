...
- `MAPILLARY_TOKEN` — только для пасхалки «Гео». Пусто = игра выключена и сама
  об этом пишет, остальной сайт работает. Токен бесплатный и без банковской
  карты, как его получить — в комментарии рядом со строкой. Вставляется целиком
  и без кавычек: вертикальные черты (`MLY|1234|abcd`) — часть токена.

Значения в `.env` пишутся КАК ЕСТЬ, без кавычек и экранирования: файл читают
docker compose и скрипты деплоя, и оба разбирают его построчно, а не отдают
оболочке.
...
убрать — строгая проверка лучше пина. Есть ещё `CRM_TLS_CA_FILE` (свой
