...
- `GOOGLE_MAPS_KEY` — только для пасхалки «Гео». Пусто = игра выключена и сама
  об этом пишет, остальной сайт работает. Что именно включать в Google Cloud
  Console и почему ключей два — в комментарии рядом со строкой.
