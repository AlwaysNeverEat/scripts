...
// @updateURL    https://raw.githubusercontent.com/AlwaysNeverEat/scripts/main/userscript/calculator.user.js
// @downloadURL  https://raw.githubusercontent.com/AlwaysNeverEat/scripts/main/userscript/calculator.user.js
// ==/UserScript==
...
(function () {
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onload: (r) => {
                    ...
                    catch { resolve({ status: r.status, data: null }); }
                },
                onerror: (e) => reject(e),
    ...
    // Convert DB car record → data object that renderCalculator() expects.
    function dbRecordToData(rec) {
        const fc = rec.fluid_capacities || {};
        const data = {};
        if (fc.engine)    data.engine    = { ...fc.engine };
        if (fc.automatic) data.automatic = { ...fc.automatic };
        if (fc.manual)    data.manual    = { ...fc.manual };
        if (fc.transfer)  data.transfer  = { ...fc.transfer };
        if (fc.diffFront) data.diffFront = { ...fc.diffFront };
        if (fc.diffRear)  data.diffRear  = { ...fc.diffRear };
        if (rec.motul_name) data.motulName = rec.motul_name;
        return data;
    }

    // Convert DB filter_part_numbers → calcState.filters shape
    // (pre-fills part numbers; prices stay empty — user must enter them)
    function dbFiltersToState(fpn) {
        const out = {
    ...
    function showDbBanner(car, dbRecord, onAccept, onIgnore) {
        const existing = document.getElementById('__zm_db_banner');
        if (existing) existing.remove();

        const banner = document.createElement('div');
        banner.id = '__zm_db_banner';
        banner.style.cssText = `
            position:fixed;bottom:18px;right:20px;z-index:999999;
            width:340px;background:#0f1117;color:#e8eaf6;
            border:2px solid #4caf50;border-radius:10px;
            box-shadow:0 8px 32px rgba(0,0,0,.6);font:13px Arial;
        `;
        banner.innerHTML = `
            <div style="padding:10px 14px;background:#1a2e1a;border-bottom:1px solid #2a3d2a;color:#4caf50;font-weight:bold;font-size:12px;border-radius:8px 8px 0 0">
                ✅ Эта машина есть в базе данных
            </div>
            <div style="padding:10px 14px;font-size:12px;line-height:1.6">
                <b>${escapeHtml(dbRecord.brand)} ${escapeHtml(dbRecord.model)}</b>
                ${dbRecord.generation ? ' · ' + escapeHtml(dbRecord.generation) : ''}
                <br>
                ${dbRecord.engine_code ? 'Код: <b>' + escapeHtml(dbRecord.engine_code) + '</b> · ' : ''}
                ${dbRecord.engine_volume ? dbRecord.engine_volume + 'л · ' : ''}
                ${dbRecord.year_from}${dbRecord.year_to ? '–' + dbRecord.year_to : '+'}
            </div>
            <div style="padding:0 14px 12px;display:flex;gap:8px">
                <button id="__zm_db_accept" style="flex:1;background:#4caf50;border:none;color:#fff;border-radius:6px;padding:7px 10px;cursor:pointer;font:12px Arial;font-weight:bold">
                    ✓ Использовать данные из БД
                </button>
                <button id="__zm_db_ignore" style="background:#1e2040;border:1px solid #3a3d5e;color:#a0a0b0;border-radius:6px;padding:7px 10px;cursor:pointer;font:11px Arial">
                    Игнор.
                </button>
            </div>
        `;
        ...
        document.getElementById('__zm_db_accept').onclick = () => { banner.remove(); onAccept(dbRecord); };
        document.getElementById('__zm_db_ignore').onclick = () => { banner.remove(); onIgnore(); };
        setTimeout(() => { if (banner.parentNode) banner.remove(); }, 20000);
    }

    // ── Save-to-DB modal ───────────────────────────────────────────────

    function showSaveModal(car, data) {
        const existing = document.getElementById('__zm_save_modal');
        if (existing) existing.remove();

        // Build filter_part_numbers from current calcState.filters (if any)
        const sf = calcState ? calcState.filters : null;
        const vfPart = sf && sf.vf.name && sf.vf.name !== '[нет]' ? sf.vf.name : '';
        const mfPart = sf && sf.mf.name && sf.mf.name !== '[нет]' ? sf.mf.name : '';
        const sfPart = sf && sf.sf.name && sf.sf.name !== '[нет]' ? sf.sf.name : '';

        const overlay = document.createElement('div');
        overlay.id = '__zm_save_modal';
        overlay.style.cssText = `
            position:fixed;inset:0;z-index:1000000;background:rgba(0,0,0,.7);
        ...
        overlay.innerHTML = `
            <div style="background:#0f1117;border:1px solid #2a2d3e;border-radius:12px;
                        ...
                        color:#e8eaf6;font:13px Arial;box-shadow:0 16px 48px rgba(0,0,0,.7)">
                <div style="padding:14px 18px;border-bottom:1px solid #2a2d3e;font-weight:bold;font-size:14px;
                             display:flex;align-items:center;justify-content:space-between">
                    <span>📥 Добавить машину в базу данных</span>
                    <button id="__zm_modal_close" style="background:none;border:none;color:#a0a0b0;font-size:18px;cursor:pointer">✕</button>
                </div>
                <div style="padding:16px 18px;display:flex;flex-direction:column;gap:12px">

                    <!-- Identity -->
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Марка *
                            <input id="__zm_f_brand" value="${escapeHtml(car.makeShort || car.make || '')}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Модель *
                            <input id="__zm_f_model" value="${escapeHtml(car.modelShort || car.model || '')}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Поколение
                            <input id="__zm_f_gen" value=""
                                placeholder="напр. IV, B9 — необязательно"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Код двигателя
                            <input id="__zm_f_ec" value="${escapeHtml(car.engineCode || '')}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Объём (л)
                            <input id="__zm_f_vol" type="number" step="0.1" min="0"
                                value="${car.volume || ''}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Тип топлива
                            <input id="__zm_f_fuel" value="${escapeHtml(car.fuelType || '')}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            кВт
                            <input id="__zm_f_kw" type="number" value="${car.kw || ''}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            л.с.
                            <input id="__zm_f_bhp" type="number" value="${car.bhp || ''}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Год с *
                            <input id="__zm_f_yf" type="number" min="1950" max="2050"
                                value="${car.yearFrom || ''}"
                                style="${inputStyle()}"/>
                        </label>
                        <label style="display:flex;flex-direction:column;gap:4px;font-size:11px;color:#a0b0c0">
                            Год по (пусто = до сих пор)
                            <input id="__zm_f_yt" type="number" min="1950" max="2050" value=""
                    ...
                    <!-- Filters — REQUIRED -->
                    <div style="border-top:1px solid #2a2d3e;padding-top:12px">
                        <div style="font-size:11px;font-weight:bold;color:#ff9800;margin-bottom:8px">
                            🔧 Фильтры двигателя — все три обязательны
                        </div>
                        ${filterRow('vf', 'Масляный фильтр (вф)', vfPart)}
                        ${filterRow('mf', 'Воздушный фильтр (мф)', mfPart)}
                        ${filterRow('sf', 'Салонный фильтр (сф)',  sfPart)}
                    </div>

                    <div id="__zm_save_err" style="color:#f44336;font-size:11px;display:none"></div>

                    <button id="__zm_save_submit" style="
                        background:#E67E00;border:none;color:#fff;border-radius:8px;
                        padding:10px 18px;cursor:pointer;font:14px Arial;font-weight:bold;
                        opacity:0.5;pointer-events:none;margin-top:4px">
                        Сохранить в базу
                    </button>
                </div>
            </div>
        ...
        document.getElementById('__zm_modal_close').onclick = () => overlay.remove();
        overlay.onclick = (e) => { if (e.target === overlay) overlay.remove(); };

        // Validation & submit
        const allInputs = overlay.querySelectorAll('input');
        allInputs.forEach(inp => inp.addEventListener('input', validateModal));
        overlay.querySelectorAll('[data-absent]').forEach(chk => {
            chk.addEventListener('change', () => {
                const key = chk.dataset.absent;
                const partInput = document.getElementById('__zm_fp_' + key);
                if (partInput) {
                    partInput.disabled = chk.checked;
                    partInput.value    = chk.checked ? '' : partInput.dataset.prev || '';
                }
                validateModal();
            });
        });
        ...
        document.getElementById('__zm_save_submit').onclick = async () => {
            const btn = document.getElementById('__zm_save_submit');
            btn.textContent = 'Сохранение…';
            btn.style.opacity = '0.7';

            const body = buildSaveBody(car, data);
            if (!body) return;
            ...
            try {
                ...
                if (resp.status === 201 || resp.status === 200) {
                    ...
                    showModalError(resp.data?.error || `Ошибка ${resp.status}`);
                    btn.textContent = 'Сохранить в базу';
                    btn.style.opacity = '1';
                }
            } catch (e) {
                showModalError('Нет связи с сервером: ' + e.message);
                btn.textContent = 'Сохранить в базу';
                btn.style.opacity = '1';
            }
        };
        ...
        function validateModal() {
            const brand   = document.getElementById('__zm_f_brand').value.trim();
            const model   = document.getElementById('__zm_f_model').value.trim();
            const yearFrom = document.getElementById('__zm_f_yf').value.trim();
            let filtersOk = true;
            for (const key of ['vf', 'mf', 'sf']) {
                const absent = document.getElementById('__zm_abs_' + key)?.checked;
                const part   = document.getElementById('__zm_fp_' + key)?.value.trim();
                if (!absent && !part) { filtersOk = false; break; }
            }
            const ok = brand && model && yearFrom && filtersOk;
            const btn = document.getElementById('__zm_save_submit');
            if (btn) {
        ...
        function buildSaveBody(car, data) {
            const fpn = {};
            for (const key of ['vf', 'mf', 'sf']) {
                const absent = document.getElementById('__zm_abs_' + key)?.checked;
                const part   = document.getElementById('__zm_fp_' + key)?.value.trim();
                fpn[key] = absent ? { part: null, absent: true } : { part, absent: false };
            }
            return {
                brand:         document.getElementById('__zm_f_brand').value.trim(),
                model:         document.getElementById('__zm_f_model').value.trim(),
                generation:    document.getElementById('__zm_f_gen').value.trim() || null,
                engine_code:   document.getElementById('__zm_f_ec').value.trim() || null,
                engine_volume: parseFloat(document.getElementById('__zm_f_vol').value) || null,
                kw:            parseInt(document.getElementById('__zm_f_kw').value) || null,
                bhp:           parseInt(document.getElementById('__zm_f_bhp').value) || null,
                fuel_type:     document.getElementById('__zm_f_fuel').value.trim() || null,
                year_from:     parseInt(document.getElementById('__zm_f_yf').value),
                year_to:       parseInt(document.getElementById('__zm_f_yt').value) || null,
                motul_name:    data?.motulName || null,
        ...
        function buildFluidCapacities(data) {
            if (!data) return {};
            const fc = {};
            for (const k of ['engine','automatic','manual','transfer','diffFront','diffRear']) {
                if (data[k]) fc[k] = { ...data[k] };
            }
            return fc;
        }

        function showModalError(msg) {
            const el = document.getElementById('__zm_save_err');
            if (el) { el.textContent = msg; el.style.display = 'block'; }
        }
    }

    function inputStyle() {
        return 'background:#1a1d2e;border:1px solid #2a2d3e;color:#e8eaf6;border-radius:6px;padding:6px 8px;font:13px Arial;width:100%;box-sizing:border-box';
    }

    function filterRow(key, label, prefill) {
        return `
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <span style="width:140px;font-size:11px;color:#a0b0c0;flex-shrink:0">${label}</span>
                <input id="__zm_fp_${key}" value="${escapeHtml(prefill)}"
                    data-prev="${escapeHtml(prefill)}"
                    placeholder="артикул, напр. W7023"
                    style="${inputStyle().replace('width:100%','flex:1')}"/>
                <label style="display:flex;align-items:center;gap:4px;font-size:11px;white-space:nowrap;cursor:pointer">
                    <input type="checkbox" id="__zm_abs_${key}" data-absent="${key}"/>
                    нет
                </label>
            </div>
        `;
    ...
    function initMann(source) {
        ...
        let dbMatchChecked = {}; // cacheKey → 'matched'|'ignored'|'accepted'
        ...
        function render() {
            ...
            let cached;
            const dbKey = 'db_car_' + car.cacheKey;
            const usingDb = dbMatchChecked[car.cacheKey] === 'accepted';

            if (usingDb) {
                // Use data that was loaded from DB
                const stored = GM_getValue(dbKey, null);
                cached = stored ? JSON.parse(stored) : null;
            } else if (source === 'ravenol') {
            ...
            if (expanded) {
                ...
                // "Добавить в БД" button in calculator header
                bindSaveButton(car, cached);
                // "Обновить из Mann" button when using DB data
                if (usingDb) bindRefreshFromMannButton(car);
            } else {
                widget.classList.remove('zm-full');
                widget.innerHTML = shellHTML(renderTrayBody(car, cached, usingDb));
                bindHeaderEvents(car);
                bindTrayEvents(car, usingDb);
            }

            lastRenderedKey = car.cacheKey + '|' + (cached ? 'Y' : 'N') + '|R' +
                ((GM_getValue('rolf_approvals_'+car.cacheKey, null)||[]).length) +
                '|' + (expanded ? 'E' : 'T') + '|' + (usingDb ? 'DB' : '');

            // Trigger DB match check on first encounter of this car
            if (!dbMatchChecked[car.cacheKey] && API_BASE && API_BASE !== 'https://your-backend.railway.app') {
                dbMatchChecked[car.cacheKey] = 'checking';
                checkDbMatch(car);
            }
        }

        async function checkDbMatch(car) {
            try {
                ...
                const resp = await dbRequest('GET', '/api/cars/match?' + params.toString());
                if (resp.status === 200 && resp.data) {
                    const rec = resp.data;
                    showDbBanner(car, rec,
                        (dbRecord) => {
                            // User accepted: store data and use it
                            const data = dbRecordToData(dbRecord);
                            GM_setValue('db_car_' + car.cacheKey, JSON.stringify(data));
                            dbMatchChecked[car.cacheKey] = 'accepted';
                            // Pre-fill filter part numbers if calcState is already open
                            if (calcState) {
                                calcState.filters = dbFiltersToState(dbRecord.filter_part_numbers);
                                calcState.filtersRaw = '';
                            }
                            expanded = true;
                            render();
                        },
                        () => {
                            // User ignored
                            dbMatchChecked[car.cacheKey] = 'ignored';
                        },
                    );
                } else {
                    dbMatchChecked[car.cacheKey] = 'no_match';
                }
            } catch (_) {
                dbMatchChecked[car.cacheKey] = 'error';
            }
        }

        function bindSaveButton(car, data) {
            const btn = document.getElementById('zm-save-to-db');
            if (btn) btn.onclick = () => showSaveModal(car, data);
        }

        function bindRefreshFromMannButton(car) {
            const btn = document.getElementById('zm-refresh-from-mann');
            if (btn) btn.onclick = () => {
                if (!confirm('Переискать на Mann/Motul и обновить запись в БД после результата?')) return;
                dbMatchChecked[car.cacheKey] = 'ignored';
                GM_deleteValue('db_car_' + car.cacheKey);
                expanded = false;
                openMotulSearch(car);
                render();
            };
        ...
        function renderTrayBody(car, cached, usingDb) {
            const rolfApp = GM_getValue('rolf_approvals_' + car.cacheKey, null);
            const motulOk = !!cached;
            const rolfOk = !!(rolfApp && rolfApp.length);

            const sourceLabel = usingDb
                ? '🗄 <span style="color:#4caf50">данные из БД</span>'
                : (motulOk ? '✓ <span class="zm-ok">Motul</span>' : '<span class="zm-wait">○ Motul (объёмы)</span>');

            const status = `
                <div style="font-size:11px;line-height:1.7">
                    <div>${sourceLabel}</div>
                    ${!usingDb ? `<div>${rolfOk ? '✓ <span class="zm-ok">ROLF</span> (' + rolfApp.length + ' допусков)' : '<span class="zm-wait">○ ROLF (допуски)</span>'}</div>` : ''}
                </div>
            `;
            ...
            return `
                ...
                <div class="zm-tray-btns" style="flex-direction:column;gap:4px">
                    ${!motulOk && !usingDb ? `<button class="zm-btn zm-btn-pri" id="zm-search">🔍 Найти на Motul</button>` : ''}
                    ${!usingDb ? `<button class="zm-btn zm-btn-sec" id="zm-rolf">📋 Допуски (ROLF)</button>` : ''}
        ...
        function shellHTML(body) {
            const headerRight = expanded
                ? `<button class="zm-btn zm-btn-sec" id="zm-rolf-exp" title="Допуски ROLF">📋 ROLF</button>
                   ...
                   <button class="zm-btn" style="background:#1a3d1a;border:1px solid #2a5d2a;color:#81c784;font-size:11px" id="zm-save-to-db" title="Добавить машину в базу данных">📥 В БД</button>
                   <button class="zm-btn zm-btn-sec" id="zm-refresh-from-mann" style="display:none" title="Обновить из Mann">↺ Mann</button>
        ...
        function bindTrayEvents(car, usingDb) {
            ...
            const saveBtn = document.getElementById('zm-save-to-db');
            if (saveBtn) saveBtn.onclick = () => {
                const dataKey = usingDb ? 'db_car_' : 'motul_car_';
                const stored = GM_getValue(dataKey + car.cacheKey, null);
                const data = stored ? JSON.parse(stored) : null;
                showSaveModal(car, data);
            };
