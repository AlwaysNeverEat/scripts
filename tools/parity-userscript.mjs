...
for (const c of CASES) {
    ...
    // в отчёте переименована с «+ 550р (с\у\з\к)» на «+ 550₽ (картер)» — это
    // синонимы. База (44f7180) всё ещё печатает старый вариант; нормализуем его
    // к новому, чтобы паритет продолжал стеречь остальной формат отчёта.
    const originalNorm = original.replace(/ \+ 550р \(с\\у\\з\\к\)/g, ' + 550₽ (картер)');
