...
function boot() {
    ...
    const module = {
        ...
        // почему надо.
        print: () => {},
        // prboom при старте зовёт SDL_WM_SetCaption («PrBoom 2.5.0»), а
        // emscripten по умолчанию превращает это в document.title — и вкладка
        // сайта переименовывается в игру, причём НАВСЕГДА: экземпляр живёт
        // дольше окна, и возвращать заголовок было бы некому. Окно у игры тут
        // не своё, а наше, поэтому имя вкладки она не трогает.
        setWindowTitle: () => {},
        setStatus: text => {
