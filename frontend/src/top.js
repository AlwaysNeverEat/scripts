...
function render(body, data) {
    ...
    // Строки топа лежат на ПОДЛОЖКЕ, а не прямо на фоне: между ними всего одна
    // волосяная линия, и на обоях (стеклянная тема) от списка остались бы
    // висящие в воздухе буквы. Заголовок месяца при этом остаётся снаружи —
    // подпись к группе не поднимается на её подложку (см. .sec-card).
    body.innerHTML = previousHtml(previous)
        + (label ? `<div class="top-month">${esc(label)}</div>` : '')
        + `<div class="sec-card top-list">${listHtml}</div>`;
