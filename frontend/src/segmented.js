...
// Совпадение с точностью до полупикселя: числа приходят из
// getBoundingClientRect и на дробном масштабе экрана не бывают равны точно.
const same = (a, b) => a && b
    && Math.abs(a.w - b.w) < 0.5 && Math.abs(a.h - b.h) < 0.5
    && Math.abs(a.x - b.x) < 0.5 && Math.abs(a.y - b.y) < 0.5;

function put(pill, box, animate) {
    // ПИЛЮЛЯ НА МЕСТЕ — НЕ ТРОГАЕМ ЕЁ ВООБЩЕ, и это не экономия на записи в
    // стиль, а единственное, что спасает начавшийся переезд.
    //
    // Смена выбора приходит СРАЗУ ДВУМЯ путями. Первым — наблюдателем за
    // классом: он микрозадача, срабатывает мгновенно и запускает поездку.
    // Вторым — ResizeObserver: у активного чипа своя насыщенность шрифта, от
    // неё меняется ширина дорожки, и наблюдатель за размером просыпается в том
    // же кадре. Он ставил пилюлю НА ТО ЖЕ САМОЕ место, но с `transition: none`
    // — то есть гасил переезд за миг до первого его кадра. Со стороны это
    // выглядело как «анимация не работает»: пилюля просто оказывалась на новом
    // месте. Раньше спасал порядок (перерисовка ставила пилюлю из rAF, уже
    // после наблюдателя за размером), но стоило перерисовке уйти — и переезд
    // гас каждый раз.
    if (same(pill.__box, box)) {
        // ГЕОМЕТРИЯ ТА ЖЕ — НО ПИЛЮЛЯ МОГЛА БЫТЬ ПОГАШЕНА. Пока раздел скрыт
        // (переключились на другую вкладку сайта — там display: none), группа
        // меряется нулём, и place() прячет пилюлю, чтобы она не висела в углу.
        // На возврате ResizeObserver будит нас с ТЕМ ЖЕ местом, и пропусти мы
        // запись целиком — пилюля осталась бы невидимой до первого клика.
        // Возвращать её на экран нужно всегда, а вот двигать — нечего.
