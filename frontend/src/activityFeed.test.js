...
test('по умолчанию лента открыта на последних неделях, а не на целом годе', () => {
    ...
    // Ползунок обязан показывать ровно тот же сдвиг и упираться в него.
    assert.match(html, new RegExp(`max="${pan}"[^>]*value="${pan}"`));
    // Подпись периода кончается сегодняшним днём — это и есть «где мы».
    assert.match(html, /7 сентября 2026<\/span>/);
});

test('данных меньше масштаба — ползунка нет вовсе', () => {
    // Новый аккаунт: записи за месяц. Двигать окно некуда, и дорожка, по
    // которой ничего не едет, выглядела бы поломкой.
