// ── WarpText: надпись, которую ведёт шейдер ──────────────────────────────────
//
// Это ПОРТ, а не «по мотивам»: компонент взят с reactbits.dev целиком
// (https://reactbits.dev/text-animations/warp-text, MIT,
// react-bits/src/content/TextAnimations/WarpText/), вместе с ogl. Шейдеры,
// растеризация надписи в канвас, числа искажения, поведение указателя и разбор
// жизненного цикла перенесены как есть — правки в них надо сверять с
// оригиналом, ровно как у труб (design/pipes/README.md).
//
// Разошлось ровно три вещи, и все три — из-за того, что у нас не React:
//
// 1. Вместо компонента — функция `createWarpText(container, props)`, тело
//    useEffect стало телом функции, а его cleanup — методом `destroy()`.
// 2. Отсчёт времени и положение указателя лежат НА МОДУЛЕ, а не на экземпляре.
//    В React компонент живёт, пока открыт экран; у нас раздел «Записи»
//    перерисовывается целиком раз в 45 секунд (см. render() в records.js), и
//    свой startTime у каждого экземпляра означал бы, что шум искажения
//    сбрасывается в ноль на каждой перерисовке — буквы дёргались бы сами по
//    себе. Экземпляр на странице всё равно один.
// 3. Цвет надписи мы пересчитываем на ходу (`update({ color })`) — оригиналу
//    это даёт React, а нам нужно на смену темы.
//
// Текст в разметке НЕ ЗАМЕНЯЕТСЯ канвасом: канвас кладётся ПОВЕРХ живой
// надписи, и прячет её вызывающий — только после того, как контекст WebGL
// действительно поднялся. Не поднялся (нет WebGL, не приехал чанк) — на экране
// остаётся обычный заголовок, то есть худшее, что бывает, это «как было».
...
// Значения по умолчанию — те же, что у пропсов компонента.
const DEFAULTS = {
...
// Общие на модуль, а не на экземпляр — см. шапку (пункт 2).
const epoch = performance.now();
const pointer = { x: 0.5, y: 0.5, tx: 0.5, ty: 0.5, active: 0, activeTarget: 0 };

/**
 * Поднимает эффект в готовом узле-сцене: канвас кладётся в него `absolute`,
 * поэтому узел должен быть `position: relative|absolute` и иметь размер.
 *
 * @returns {{update:(patch:object)=>void, destroy:()=>void}|null} — null, если
 *          WebGL не поднялся (звать нечего, надпись остаётся обычной).
 */
export function createWarpText(container, options = {}) {
    ...
    return {
        // Место React-эффекта, следящего за пропсами: меняем и перерисовываем.
        update(patch = {}) {
            if (disposed) return;
