...
export const DEFAULT_THEME = Object.freeze({
    base: 'dark',        // под каким стеклом: 'dark' | 'light'
    accent: '#d4a017',   // акцент сайта и цвет своей строки в топе
    preset: 'aurora',    // фон, когда своей картинки нет
    background: null,    // URL загруженной картинки — важнее пресета
    dim: 45,             // затемнение фона под стеклом, 0…85 %
...
export function normalizeTheme(raw) {
    ...
    const background = typeof src.background === 'string' && URL_RE.test(src.background.trim())
        ...
        : null;
    return {
        base: src.base === 'light' ? 'light' : 'dark',
        accent,
        preset,
        background,
