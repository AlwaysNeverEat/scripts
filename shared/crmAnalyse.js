...
const BRAND_ALIASES = [
    ...
    // Своё масло в номенклатуре CRM пишут и латиницей, и кириллицей («СПОТ
    // Оптимал»): его заводили руками, а не выгрузкой поставщика. Пока
    // кириллицы тут не было, ни одна строка своего масла не сопоставлялась с
    // каталогом — и калькулятор писал «нет на станции» на бочку, которая
    // стоит в каждом цеху.
    //
    // \b в JS считает границей только ASCII-слово, поэтому для кириллицы она
    // не работает вовсе: /\bСПОТ\b/ не совпадает ни с чем. Границу задаём
    // явным классом символов.
    ['SPOT',       /\bSPOT\b|(^|[^A-ZА-ЯЁ0-9])СПОТ([^A-ZА-ЯЁ0-9]|$)/],
];
...
// Кириллическое написание названия → латинское из каталога. Общего
// транслитератора тут мало: «ПРОФЕШНЛ», «ПРОФЕШЕНАЛ» и «ПРОФЕССИОНАЛ» — это
// одно и то же слово, написанное тремя разными людьми, а побуквенный перевод
// развёл бы их в три разных токена. Список короткий нарочно: он про наше
// масло, а не про весь каталог.
const TOKEN_SYNONYMS = new Map([
    ['ОПТИМАЛ',      'OPTIMAL'],
    ['ОПТИМАЛЬНОЕ',  'OPTIMAL'],
    ['ПРОФЕШНЛ',     'PROFESSIONAL'],
    ['ПРОФЕШЕНАЛ',   'PROFESSIONAL'],
    ['ПРОФЕШИОНАЛ',  'PROFESSIONAL'],
...
export function stripCrmCode(name) {
    // Дефис и слэш — тоже части кода («104787-200 Моторное масло ROLF…»);
    // вязкость «5W-30» под них не попадает благодаря проверке VISC_RE.
...
export function isBulkOil(name) {
    ...
    // Ведущий код бывает с дефисом и слэшем («104787-200 Моторное масло ROLF…») —
    // без них такая позиция считалась штуками, а цена показывалась за 0.1 л.
