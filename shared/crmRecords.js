...
// Сеть работает до 21:00, поэтому последняя запись начинается в 20:30 и
// заканчивается ровно к закрытию. Оригинальная админка рисует «Добавить»
// вплоть до 22:30 (её сетка шире рабочего дня) — эти слоты клон не предлагает
// ни в сетке станции, ни в выборе времени, ни в очереди операций.
export const WORK_END_MIN = 21 * 60;

// Последнее время начала записи — '20:30'. Показываем его в подсказках.
export const LAST_START_TIME = minToTime(WORK_END_MIN - SLOT_MINUTES);

// Можно ли начать запись в этом слоте: получас целиком должен уместиться до
// закрытия. Уже существующие записи после 21:00 (их заводили мимо клона) это
// не прячет — они рисуются как есть, просто новых там не создать.
export function isBookableTime(time) {
    const min = timeToMin(time);
    return Number.isFinite(min) && min + SLOT_MINUTES <= WORK_END_MIN;
}
...
// «Мусорный» телефон: номер, набранный одной и той же цифрой, — +7 111 111-11-11,
// +7 000 000-00-00, 8 999 999-99-99. Так выглядит и заглушка продолжения, и
// запись, поставленную «для галочки» без клиента. В топ такие не идут
// (backend/src/records/sync.js): очко за запись должно означать живого
// человека, которого действительно записали, иначе счётчик накручивается
// десятком записей на «+7 111…» за минуту.
//
// Пустой телефон мусором НЕ считается: запись без номера — обычное дело
// (клиент подошёл на станцию), и она честно сделана.
export function isJunkPhone(phone) {
    const d = normPhoneDigits(phone);
    if (!d) return false;
    // Национальная часть — 10 цифр после кода страны; у номера без кода
    // смотрим на всё, что есть.
    const national = d.length === 11 ? d.slice(1) : d;
