...
// одиночками, прилипшими к ней по краям (см. findGroups), и ОНИ ЖЕ ИДУТ В СЧЁТ
// награды: четыре фишки одного вида, ушедшие разом, — ракета, легли они в ряд
// или углом (см. rewardFor). Линия важна ровно в одном месте: пятёрка в ряд —
// призма, потому что так эту игру знают.
//
...
// Сколько фишек должно уйти РАЗОМ, чтобы за область дали награду. Считаются все
// убранные фишки области, а не только выстроенные в линию: игрок видит на поле
// пятно из четырёх фишек одного вида и ждёт за него ракету — «эти три в счёт, а
// четвёртая сбоку нет» с поля не читается никак (см. rewardFor).
//
// Пороги не подряд (4, 6, 8) намеренно. Пятно набирается легче линии: к тройке
// сама собой прилипает в среднем ещё фишка, поэтому четвёрка — это почти каждый
// ход, а шестёрка уже редкость. Прогон шестидесяти партий по двести ходов: с
// такими порогами на сотню ходов приходится 86 ракет, 8 бомб и 1.6 призмы, то
// есть сильные фишки остались примерно такой же редкостью, какой были, когда
// награду считала одна форма (4.6 и 1.1), а ракета стала обычным делом — за неё
// и была жалоба.
export const ROCKET_TILES = 4;
export const BOMB_TILES = 6;
export const PRISM_TILES = 8;
...
export const POINTS_PER_LEVEL = 2_500;
const CLOCK_CHANCE_START = 0.06;
const CLOCK_CHANCE_STEP = 0.008;
const CLOCK_CHANCE_FLOOR = 0.02;
 ...
 *
 * Награду даёт ЧИСЛО убранных фишек области — всё, что игрок видит исчезающим
 * разом, а не только выстроенное в линию. Четыре фишки одного вида — ракета,
 * легли они в ряд, квадратом или углом: с поля «эти три в счёт, а четвёртая
 * сбоку нет» не читается никак, а в легенде написано просто «четвёрка». Раньше
 * награду считала одна лишь ФОРМА, и пятно из четырёх не давало НИЧЕГО в пяти
 * случаях из шести — четвёрок в ряд среди них меньшинство.
 *
 * Пороги бомбы и призмы (BOMB_TILES, PRISM_TILES) выше четвёрки на всё те же
 * прилипшие одиночки: пятно набирается само, линия — нет. Иначе сильные фишки
 * сыпались бы за случайность, а не за построенное.
 *
 * Поэтому же ФОРМА осталась вторым, независимым основанием для награды — она
 * называет то, что игрок собрал НАМЕРЕННО, и с числом фишек не спорит, а идёт
 * рядом: пятёрка в ряд — призма, пересечение линий (угол, Т, крест) — бомба,
 * сколько бы фишек в области ни было. Обе эти фигуры нарисованы в легенде, их
 * строят специально, и потерять их из-за порога по размеру было бы обиднее
 * всего. Берём то из двух оснований, что даёт СИЛЬНЕЕ.
 *
...
function rewardFor(game, group) {
    ...
    // Пересечение линий: одна область, в которой есть и строка, и столбец.
    const crossed = group.runs.some(r => r.horiz) && group.runs.some(r => !r.horiz);

    // Направление ракеты берём у ЛИНИИ: четвёрка (да и тройка с прилипшей сбоку)
    // лежит на поле вдоль строки или вдоль столбца, туда фишка и полетит — на
    // одну и ту же картинку ответ один и тот же, как её ни собери. Спорить тут
    // нечему: область с линиями в обе стороны — это уже бомба, не ракета.
    // Линии нет только у квадрата 2×2; у него направление берётся у ХОДА
    // (двигал по горизонтали — полетит по строке), потому что «случайно куда»
    // игрок прочесть не может.
    const rocket = longest
        ? (longest.horiz ? ROCKET_H : ROCKET_V)
        : (swapWasVertical(game) ? ROCKET_V : ROCKET_H);

    let special = NONE;
    if (size >= PRISM_TILES || (longest && longest.len >= 5)) special = PRISM;
    else if (size >= BOMB_TILES || crossed) special = BOMB;
    else if (size >= ROCKET_TILES) special = rocket;
