...
// Луза — это ДВА размера, и путать их нельзя.
//
// POCKET_R — радиус ЗАХВАТА: шар падает, когда его ЦЕНТР подошёл к точке лузы
// ближе этого. POCKET_MOUTH — ширина выреза в борте: пока шар в нём, борта для
// него нет, и шар может уехать за линию сукна (там его и ловит захват).
//
// Связь между ними НЕ СЛУЧАЙНА: устье ровно на радиус шара больше захвата.
// Тогда шар пропадает В ТОТ МОМЕНТ, когда целиком вошёл в нарисованную лузу, —
// а не когда наполовину торчит из неё. Первая версия ловила шар по центру в
// круге, равном нарисованному, и без выреза в борте: получалось, что лунка
// выглядит вдвое больше своей рабочей области — шар в неё «влезал», отскакивал
// от борта поперёк устья и уезжал обратно на стол. Именно это и было видно.
//
// САМО ЧИСЛО ВЗЯТО ИЗ КАРТИНКИ БОРТА, а не выбрано: лузы нарисованы в ней, и
// вырез средней лузы у кромки имеет полуширину 3.83 дюйма (замер в
// design/pool-table/README.md). Отсюда устье 3.83, а захват — на радиус шара
// меньше. Перерисуют борт с другими лузами — это число надо пересчитать, иначе
// игра снова начнёт отбивать шары от невидимого борта посреди лунки.
export const POCKET_R = 2.7;
export const POCKET_MOUTH = POCKET_R + BALL_R;
...
// Замедление от сукна, дюймы за секунду в квадрате. Настоящее сопротивление
// качению вчетверо меньше, но настоящий и стол: там шар видно живьём, а на
// экране медленный выкат читается не как «катится», а как «подвисло».
const DECEL = 20;

// Сила удара: 0 — самый мягкий накат, 1000 — разбой.
//
// ЧИСЛА ЗДЕСЬ ПОДОБРАНЫ ЗАМЕРАМИ, а не на глаз, и мерить надо ДЛИНУ ПУТИ шара, а
// не то, где он в итоге остановился: он отражается от бортов и возвращается,
// поэтому по конечной точке выходит ерунда.
//
// Что дают текущие: слабейший удар проезжает 1.2 длины стола, средний (половина
// замаха) — 3.2, разбой — около 7. Первая версия шкалы (30..340 при трении 18)
// давала 0.3 длины на слабейшем: нижняя треть ползунка была неотличима от
// «толкнул», а разброс между краями — семнадцатикратным. Сейчас он шестикратный,
// и каждая четверть замаха что-то различимо меняет.
const SPEED_MIN = 65;
const SPEED_MAX = 340;

// Сколько скорости остаётся после борта и после шара. Борт ест больше — он
// мягкий, — но не половину: при 0.72 шар от борта буквально умирал, и стол
// казался ватным.
const CUSHION_KEEP = 0.82;
const BALL_KEEP = 0.96;

// Ниже этой скорости шар считается остановившимся. Без порога он «ползёт»
// бесконечно: трение линейное, а скорость к нулю подходит асимптотически. Три
// дюйма в секунду — это чуть больше диаметра шара за секунду: на глаз такой шар
// уже стоит, а удар из-за него не тянется лишние полторы секунды.
const STOP_SPEED = 3;
...
/**
 * Какой силой бить, чтобы шар проехал примерно столько дюймов.
 *
 * Обратна тому, как сила превращается в скорость в shoot(). Живёт ЗДЕСЬ, а не у
 * бота, который ею и пользуется: это те же три константы (трение и края шкалы), и
 * второй их копией они бы разошлись с физикой при первой же правке — бот
 * продолжал бы рассчитывать силу по старому столу.
 *
 * Точность тут не нужна: варианты силы всё равно перебираются с шагом, а решает
 * полный расчёт удара.
 */
export function powerForDistance(inches) {
    const v = Math.sqrt(Math.max(0, inches) * 2 * DECEL);           // v² = 2·a·s
    const t = Math.sqrt(Math.max(0, (v - SPEED_MIN) / (SPEED_MAX - SPEED_MIN)));
...
export function canPlaceCue(game, x, y) {
    ...
    // В устье лузы биток тоже не ставят: там нет борта, и он бы сразу упал.
    for (const p of POCKETS) {
        const dx = p.x - x;
        const dy = p.y - y;
        if (dx * dx + dy * dy < POCKET_MOUTH * POCKET_MOUTH) return false;
    }
    return true;
}
...
/** Шар в устье лузы — там борта для него нет. */
function inMouth(b) {
    for (const p of POCKETS) {
        const dx = b.x - p.x;
        const dy = b.y - p.y;
        if (dx * dx + dy * dy <= POCKET_MOUTH * POCKET_MOUTH) return true;
    }
    return false;
}
...
function advance(game, dt) {
    ...
    // 2. Лузы — раньше бортов: шар, дошедший до устья, должен упасть, а не
    //    оттолкнуться от борта поперёк лузы.
    for (const b of balls) {
        if (b.in) continue;
        // Уехал за линию сукна — значит, прошёл сквозь вырез в борте, и деваться
        // ему больше некуда: в других местах борт бы его не выпустил.
        let fell = b.x < 0 || b.x > TABLE_W || b.y < 0 || b.y > TABLE_H;
        if (!fell) {
            for (const p of POCKETS) {
                const dx = b.x - p.x;
                const dy = b.y - p.y;
                if (dx * dx + dy * dy <= POCKET_R * POCKET_R) { fell = true; break; }
            }
        }
        if (!fell) continue;
        ...
        // Упавший шар засчитывается как «дошёл до борта»: правило про борт
        // существует, чтобы нельзя было толкнуть шар на полдюйма и отдать ход, а
        // забитый шар — это точно не оно.
        if (game.ev.firstHit != null) game.ev.railAfterHit = true;
    }

    // 3. Борта — кроме тех мест, где в них вырез под лузу.
    for (const b of balls) {
        if (b.in) continue;
        if (inMouth(b)) continue;
        let hit = false;
