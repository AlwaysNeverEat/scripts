...
// сказали, считать незачем. Поэтому наружу уезжает id панорамы, а лат/лон
// загаданного места живёт на сервере до конца раунда.
//
// Полностью спрятать место всё равно нельзя: панораму рисует гугловский
// скрипт в том же браузере, и человек с открытой консолью вытащит из него
// координату. Это ровно та же цена, что у остальных пасхалок (см. шапку
// shared/tetris.js): защита от мусора и от случайного дубля, а не от того, кто
// специально сел ломать свой же мини-топ.
//
...
// Ближе этого — сразу максимум. Панорама снята с машины, и «ткнул точно в
// точку съёмки» на карте мира неотличимо от «ткнул в тот же перекрёсток».
...
// перед панорамой и не может ни доиграть, ни выйти с победой. Две минуты — это
