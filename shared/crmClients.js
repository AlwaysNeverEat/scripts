...
// Номер добран до конца — больше в него не влезет ни одного символа.
//
// Отдельно от plateComplete, и разница принципиальная: регион бывает и
// двузначным, и трёхзначным, поэтому «К926АА14» — уже валидный номер, но
// человек, скорее всего, ещё печатает третью цифру. Всё, что решает «пора
// идти в CRM», обязано различать эти два состояния: уйти на восьмом символе
// значит поискать НЕСУЩЕСТВУЮЩИЙ номер и мигнуть «ничего не найдено» ровно
// тогда, когда человек ещё набирает.
export function plateFull(value) {
    return formatPlateInput(value).length === PLATE_SHAPE.length;
}

// Общий вопрос к строке поиска: «дальше вводить нечего?». У телефона это
// совпадает с «номер набран» (десять цифр и всё), у гос. номера — нет.
export function searchValueFull(kind, value) {
    return kind === 'plate' ? plateFull(value) : phoneComplete(value);
}
